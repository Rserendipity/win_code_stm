...
void OLED_ShowTIttle1(u8 x, u8 y)
{
	u8 t, i, adder = 0;
...
//x,y :起点坐标(x:0~127 y:0~63)
********************************************/
void OLED_ShowCHinese1(u8 x, u8 y)
{      			    
	u8 t, i, adder = 0, flag = 0;
...
void OLED_ShowCHinese2(u8 x, u8 y)
{      			    
	u8 t, i, adder = 0, flag = 0;
	...
	for(i = 0; i < 12; i++)
	{
		...
		for(t = 0; t < 16; t++)
		{
			OLED_WR_Byte(chan1[i][t], OLED_DATA);
			adder += 1;
		}
	}
} 
